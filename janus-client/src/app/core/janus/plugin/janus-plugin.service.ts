...
export class JanusPluginService {
    ...
    const pluginObject = {
      ...
      onmessage: (msg: OnMessage, jsep: any) => {
        ...
        if (jsep) {
          ...
          if (pluginHandle) {
            // pluginHandle.pluginHandle.handleRemoteJsep({ jsep: jsep });
  ...
  detachPlugin(pluginId: any): void {
    const pluginHandleId = this.getPluginHandle(pluginId)
  ...
  async replaceMedia(pluginNameOrId: any): Promise<void> {
    // const pluginHandel = this.getPluginHandle(pluginNameOrId)


    // const deviseList = await navigator.mediaDevices.enumerateDevices();
    // console.log(deviseList.filter(device => device.kind === 'videoinput'))

    // const obsVideoInput = deviseList.find(device => device.label === 'OBS Virtual Camera')

    // console.log(obsVideoInput)

    // const media = await navigator.mediaDevices.getUserMedia({ video: { deviceId: obsVideoInput?.deviceId }, audio: true, });

    // console.log(media)

    // pluginHandel?.pluginHandle.replaceTracks({
    //   tracks: [{
    //     type: 'video',
    //     capture: media.getVideoTracks()[0]
    //   }]
    // });

  }

  async pluginHandleCreateOffer(pluginNameOrId: any): Promise<void> {
    const pluginHandel = this.getPluginHandle(pluginNameOrId)
    ...
    pluginHandel?.pluginHandle.createOffer(
      {
        tracks: [
          ...
          // { type: 'video', capture: media.getVideoTracks()[0], recv: true }
