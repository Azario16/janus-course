...
export class JanusPluginStreamingService {
  ...
  private remoteTrackList = new BehaviorSubject<any>([]);
  remoteTrackList$ = this.remoteTrackList.asObservable();
  ...
  getVideoRoomPlugin(pluginNameOrId: any, callback = () => { }): any {
    const pluginObject = {
      plugin: "janus.plugin.streaming",
      opaqueId: 'opaque_' + pluginNameOrId,
      ...
      onremotetrack: (track: MediaStreamTrack, mid: any, added: any, metadata: any) => {
        ...
        if (metadata.reason === 'unmute') {
          ...
          // const currentRemoteTrackList = this.remoteTrackList.getValue()


          // const concatTrackList = [...currentRemoteTrackList, track]
          // this.remoteTrackList.next(concatTrackList)

          // const concatMediaStream = new MediaStream(concatTrackList)

          // console.log(concatMediaStream)
          // console.log(concatMediaStream.getTracks())



          // this.remoteStremList.next([concatMediaStream])
  ...
  hangUp(pluginId: any): void {
    this.logDebug(['hangup'])
    const pluginHandleId = this.getPluginHandle(pluginId)

    if (pluginHandleId) {
      pluginHandleId.pluginHandle.hangup((request: any) => {
    ...
    pluginHandel?.pluginHandle.createOffer(
      {
        ...
        success: (jsep: any) => {
          this.logDebug(['success', pluginNameOrId])
          ...
          pluginHandel?.pluginHandle.send({
            message: body, jsep: jsep,
            error: (error: any) => {
              this.logDebug([error])
            },
            success: (event: any) => {
              this.logDebug(['configure success'])
            }
          });
        },
        error: (error: any) => {
  ...
  private pluginHandleCreateAnswer(videoRoomPlugin: PluginHandle, jsep: any): void {
    this.logDebug(['pluginHandleCreateAnswer'])
  ...
  private destroyPluginHandle(roomId: number, pluginHandleId: PluginHandleId): void {
    pluginHandleId.pluginHandle.detach({
      success: () => {
        ...
        this.logDebug(['destroyPluginHandle success'])
      },
      error: (error) => {
        this.logDebug([
          'destroyPluginHandle success',
          'error',
          error
        ])
      }
